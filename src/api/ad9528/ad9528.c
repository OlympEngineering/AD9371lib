 ...
 * are known to work with the ADI AD9369 and AD9370 evaluation boards.  NOTE: This function will
...
ADI_ERR AD9528_initDeviceDataStruct(ad9528Device_t *device, uint32_t vcxoFrequency_Hz, uint32_t refAFrequency_Hz, uint32_t outputDeviceClock_Hz)
{
     ...
     *******************************************************************
     */
    device->pll2Settings->nDivider = 30;
    if (outputDeviceClock_Hz == 184320000){	device->pll2Settings->rfDivider = 5;}
    else{                                   device->pll2Settings->rfDivider = 3;}
    ...
    switch (outputDeviceClock_Hz){
        case 122880000: chDiv = 10; break;
        case 153600000: chDiv =  8; break;
        case 184320000: chDiv =  4; break;
        case 245760000: chDiv =  5; break;
        case 307200000: chDiv =  4; break;
        default:        chDiv = 10; break;
    }
...
ADI_ERR AD9528_initialize(ad9528Device_t *device) { //writes all registers to desired values, leaves clock outputs off, SYSREF output disabled.
     ...
     **********************************************************************
     */
    pll2_Bdiv = device->pll2Settings->nDivider / 4;
    pll2_Adiv = device->pll2Settings->nDivider % 4;
    ...
    CMB_SPIWriteByte(device->spiSettings, AD9528_ADDR_PLL2_REPLICA_CHDIV, (device->pll2Settings->nDivider / device->pll2Settings->rfDivider) -1 ); //Replica channel divider: Ndivider = rfDivider * R divider
